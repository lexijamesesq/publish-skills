...
description: >
  ...
  it was wired the day it shipped. Three probes, grown by regression only —
...
This skill owns ONE thing: proving three specific local surfaces are wired correctly at the moment it runs. It does not scan for new problems, does not audit the whole harness, and does not fix anything it finds.
...
- **Grows by regression, never speculatively.** A probe is added only after a real silent-misconfiguration incident bites. Three probes, three incidents:
...
Do not add a fourth probe without a new failure to justify it.
...
2. A regression in any of the three protected surfaces is caught by the next `/smoke` run, not by the next incident.
...
**Decision authority.**
- **Autonomous:** running all three probes; reporting PASS/FAIL per probe plus a summary line.
...
- Does NOT probe MCP servers, plugins, or anything `/system-blueprint` governs — this skill is scoped to a hook, the lint gate, and settings.json hook registration; blueprint drift belongs to `/system-blueprint`.
