...
1. **Scaffold verification** — `playbooks/gate.md` § Scaffold. Root-anchored `.gitignore` actually effective, gitleaks `[allowlist]` present if content-bearing, LICENSE+README present, every operator-config referenced by tracked machinery has a `*.sample.*` shape.
