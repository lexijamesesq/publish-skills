...
- **Canopy Triad Sync** — weekly (Thursdays), cross-team product-leads sync. Registry key `canopy-triad-sync` (`../capture-meeting/meeting-registry.sample.json`).
...
**Bramblesoft**, **Twig** — two fictional pre-Acorndyne employers, added solely to give `tag-taxonomy.md`'s grandfathered legacy `project/{employer}/*` depth-exception examples (a pre-`area/`-namespace convention) a sample-universe-conformant pair. Not otherwise part of this universe's narrative — distinct from `area/work/pinecone`, which serves the current-employer multi-employer example instead.

## Tag Examples

`area/work/acorndyne` (this universe) · `area/work/pinecone` (a second illustrative employer from `tag-taxonomy.md` — NOT part of this universe, kept distinct for multi-employer tag examples) · `person/hazel-acorn` · `person/chip-chestnut` · `topic/cachetrack` (no hyphen — canon) · `topic/gms` · `topic/ledger` · `topic/cap` · `topic/canopy-triad-sync`.
...
(the themed-sample + one-citation pattern) · `Wiki/spec/tag-taxonomy.md` (Acorndyne, Hazel Acorn, Chip Chestnut,
Pinecone as pre-existing illustrative tags) · `../capture-meeting/meeting-registry.sample.json`
...
grandfathered into `tag-taxonomy.md`'s legacy `project/*` depth-exception examples with no fictional stand-in
to swap to.
